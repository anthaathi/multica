...
//
// A runtime identity that ships a dedicated backend — i.e. New() has a case
// for it — wins over the generic protocol-family+override path. omp is the
// canonical case: its ompBackend parses omp's session header (the resume
// pointer), which the pi family backend does not emit, so routing omp through
// the pi backend would silently drop resume. Only a runtime with NO dedicated
// backend falls through to the family+override construction below.
//
// Fails closed: in the family+override path, if the protocol family backend
// does not implement backendOverrideApplicator, the descriptor's overrides
// cannot be applied and the runtime would launch under the wrong executable
// and label — so NewRuntime returns an error rather than returning an
// unmodified backend.
func NewRuntime(runtimeID string, cfg Config) (Backend, error) {
	...
	desc, _ := BuiltinRuntimeByID(runtimeID)
