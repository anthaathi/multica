...
func TestBackendFactoriesSetCommandLogProvider(t *testing.T) {
	...
	// The fork routes omp through its dedicated ompBackend (it parses omp's
	// session header), not upstream's *piBackend descriptor path — but the
	// assertion's intent holds: the log provider is the runtime id.
