...
//
// NOTE(fork): the quick-create submission path itself deliberately does NOT
// gate on the daemon CLI version (fork commit d588f8ec2 removed that gate);
// these helpers survive because the source-context quick-create flow
// (source_context.go, upstream MUL-6555) needs them to refuse captured-context
// runs on daemons too old to understand them.
//
