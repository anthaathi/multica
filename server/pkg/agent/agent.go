...
// Trae, Grok, Oh My Pi, Qwen Code, QwenPaw). It
...
type Config struct {
	ExecutablePath string            // path to CLI binary (claude, codebuddy, codex, copilot, opencode, openclaw, hermes, pi, cursor, kimi, reasonix, dsh, kiro-cli, agy, qodercli, qoderclicn, traecli, omp, grok, qwen, qwenpaw)
...
// Supported types: "claude", "codebuddy", "codex", "copilot", "opencode", "deveco", "openclaw", "hermes", "pi", "cursor", "kimi", "reasonix", "dsh", "kiro", "antigravity", "qoder", "qoderclicn", "traecli", "omp", "grok", "qwen", "qwenpaw".
//
...
// migration 202 to add qwen, migration 242 to add qoderclicn, migration 253
// to add qwenpaw, migration 254 to add reasonix, and migration 313 to add
// dsh): a custom runtime profile may only be based on a backend Multica
// officially supports.
...
// binaries independently (Qoder CN users can point the Qoder backend at a
// non-default binary instead of misrouting through Kiro/ACP with incompatible
// arguments, #4883). traecli (Trae) has a New backend, launch
...
// qwen is Qwen Code's native `qwen -p <prompt> --output-format stream-json`
// backend.
...
func New(agentType string, cfg Config) (Backend, error) {
	...
	default:
		return nil, fmt.Errorf("unknown agent type: %q (supported: claude, codebuddy, codex, copilot, opencode, deveco, openclaw, hermes, pi, cursor, kimi, reasonix, dsh, kiro, antigravity, qoder, qoderclicn, traecli, omp, grok, qwen, qwenpaw)", agentType)
	}
}
