...
// This file is the Mattermost user-binding token flow: an unbound Mattermost
...
// page, and their Mattermost user id is bound to their Multica account. It
// mirrors slack.BindingTokenService, running on the generic channel_* queries
// with channel_type='mattermost'.
...
var (
	...
	ErrBindingTokenInvalid = errors.New("mattermost: binding token invalid or expired")
	// ErrBindingAlreadyAssigned: this Mattermost user id is already bound to a
	// different Multica user (account transfer must go through explicit unbind).
	ErrBindingAlreadyAssigned = errors.New("mattermost: user id is already bound to a different user")
	// ErrBindingNotWorkspaceMember: the redeemer is not a member of the token's
	// workspace. Translated to 403 at the HTTP boundary.
	ErrBindingNotWorkspaceMember = errors.New("mattermost: redeemer is not a workspace member")
)
...
// BindingTokenService mints and redeems Mattermost binding tokens. Redemption
// is transactional: consuming the token and inserting the channel_user_binding
...
// Mint creates a single-use binding token for (installation, mmUserID) and
// returns the raw secret + expiry. The raw value must be delivered over
// Mattermost (encrypted in transit by the platform) and never logged.
func (s *BindingTokenService) Mint(ctx context.Context, workspaceID, installationID pgtype.UUID, mmUserID string) (BindingToken, error) {
...
// RedeemAndBind atomically consumes a raw token and binds the Mattermost user
// id to multicaUserID (taken from the session, never from the token). Returns
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, multicaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	if s.tx == nil {
		return RedeemedBindingToken{}, errors.New("mattermost: BindingTokenService missing TxStarter")
	}
	...
	return RedeemedBindingToken{
		WorkspaceID:      row.WorkspaceID,
		InstallationID:   row.InstallationID,
		MattermostUserID: row.ChannelUserID,
	}, nil
}
