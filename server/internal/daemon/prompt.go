...
func buildChatPrompt(task Task) string {
	...
	//
	// The history half applies to channels that ship a reader behind
	// `multica chat history` / `multica chat thread`: Slack and Mattermost
	// (handler/chat_history.go wires both h.SlackHistory and h.MattermostHistory).
	// A Feishu session has no reader, so both commands would return "no channel
	// integration" — teaching them there would send the agent down a path that
	// always fails. A Feishu run works from the context the inbound enricher
	// already injected, so it gets the awareness statement without the commands,
	// and ChatInThread — which only ever picks between those two commands — does
	// not apply to it (MUL-4899).
	if task.ChatChannelType != "" {
		...
		if task.ChatChannelType == execenv.ChannelTypeSlack || task.ChatChannelType == execenv.ChannelTypeMattermost {
