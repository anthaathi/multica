...
func buildChatPrompt(task Task) string {
	...
	//
	// The history half applies to channels that ship a live reader behind
	...
	// Every other IM surface (Feishu, WeCom, DingTalk) persists its transcript
	// to Multica's chat_message table, readable back via the non-Slack fallback
	// in handler/chat_history.go — so they can read the conversation back too,
	// but from Multica's store, not the channel. Slack additionally has
