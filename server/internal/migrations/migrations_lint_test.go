...
var legacyDuplicateMigrationStems = map[string][]string{
	...
	// Fork (anthaathi/multica) carries custom migrations (GitLab, issue sync,
	// runtime profiles, Mattermost) that intentionally share numeric prefixes
	// with upstream migrations of the same era. The runner applies migrations
	// in ascending sorted-filename order, so same-prefix pairs coexist; these
	// entries register them as accepted duplicates so the uniqueness lint passes.
	"135": {"135_comment_workspace_index", "135_gitlab_integration"},
	"136": {"136_issue_sync", "136_runtime_profile_add_traecli"},
