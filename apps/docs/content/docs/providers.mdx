...
| **Oh My Pi** | can1357 (community) | ✅ (session id from header) | ❌ | `.omp/skills/` | Dynamic discovery (`omp models --json`) |
...
[Oh My Pi](https://omp.sh/) (`omp`, [github.com/can1357/oh-my-pi](https://github.com/can1357/oh-my-pi)) is a community fork of Pi. It keeps Pi's JSON event protocol, so Multica drives it with the same `--mode json` event stream — but session identity, resume, and thinking diverge enough that omp has a dedicated backend rather than reusing Pi's. **Session resumption works** with a normal string id: omp emits a leading `{"type":"session","id":...}` header line, and Multica passes that id back with `--resume <id>` on the next turn. **MCP is not consumed** initially (omp configures MCP through its own settings). **Model selection is dynamic** via `omp models --json` (a subcommand, not Pi's `--list-models`), and **thinking level is supported** natively through omp's `--thinking` flag. Skills live in `.omp/skills/` (project) and `~/.omp/agent/skills` (global).
...
**Of the 18 tools, thirteen consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Qwen Code, Trae, and Grok**. The other five (Antigravity, Copilot, DevEco Code, Pi, Oh My Pi) accept the field but **ignore it** — no error, no warning, the config just has no effect.
