...
func (h *Handler) GetChatChannelHistory(w http.ResponseWriter, r *http.Request) {
	...
	if len(readers) > 0 {
		page, err = readChannelHistory(readers, func(reader ChatChannelHistoryReader) (channel.HistoryPage, error) {
			return reader.ChannelOverview(r.Context(), scope.sessionID, scope.historyOptions(r))
		})
	}
...
func (h *Handler) GetChatThread(w http.ResponseWriter, r *http.Request) {
	...
	page, err := readChannelHistory(readers, func(reader ChatChannelHistoryReader) (channel.HistoryPage, error) {
		return reader.Thread(r.Context(), scope.sessionID, threadID, scope.historyOptions(r))
	})
	h.respondChatHistory(w, r, scope.sessionID, page, err)
}
