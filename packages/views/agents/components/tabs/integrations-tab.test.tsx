...
describe("IntegrationsTab", () => {
  ...
  it("lets a non-admin agent owner bind Lark and DingTalk", () => {
    ...
    expect(screen.queryByTestId("wecom-bind-button")).toBeNull();
    expect(screen.queryByTestId("telegram-bind-button")).toBeNull();
    // The admin-only platforms on the merged fork (Slack, Mattermost, WeCom
    // and Telegram) fall back to the shared members note instead of a bind
    // CTA — one note per section. DingTalk now lets the agent owner connect
    // (asserted above), so it no longer renders the note here.
    const adminOnlyNotes = screen.getAllByText(
      /Only workspace owners and admins can manage this connection/i,
    );
