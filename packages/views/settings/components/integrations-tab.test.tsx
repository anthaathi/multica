...
// Fork: Mattermost rides in the messaging group alongside Slack.
vi.mock("./mattermost-tab", () => ({
  MattermostTab: () => <div>Mattermost detail</div>,
}));
...
describe("Integration directory", () => {
  it("shows live connection summaries without mounting configuration forms", () => {
    ...
    const shapes = ["lark", "slack", "mattermost", "dingtalk", "wecom", "telegram"].map(
      (channel) =>
  ...
  it("lists Mattermost (fork) in the messaging directory and opens its detail", () => {
    const { unmount } = renderWithI18n(<IntegrationsTab />);
    expect(
      screen.getByTestId("integration-channel-icon-mattermost"),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("link", { name: /Mattermost/ }));
    expect(state.push).toHaveBeenCalledWith(
      "/acme/settings?tab=integrations&integration=mattermost",
    );
    unmount();
    state.search = "tab=integrations&integration=mattermost";
    renderWithI18n(<IntegrationsTab />);
    expect(screen.getByText("Mattermost detail")).toBeInTheDocument();
  });
