...
[Oh My Pi](https://omp.sh/) (`omp`, [github.com/can1357/oh-my-pi](https://github.com/can1357/oh-my-pi)) is a community fork of Pi that keeps Pi's JSON event protocol. **Session resumption works** with a normal string id (omp emits a leading session header; Multica passes the id back with `--resume`). MCP is not consumed initially (configure MCP in omp's own settings). Model selection is dynamic (`omp models --json`), and thinking level is honoured via omp's native `--thinking` flag. Skills live in `.omp/skills/` (project) and `~/.omp/agent/skills` (global).

| | |
|---|---|
| Daemon looks for | `omp` |
| Install | `brew install can1357/tap/omp`, or `bun install -g @oh-my-pi/pi-coding-agent` (requires bun ≥ 1.3.14). See [omp.sh](https://omp.sh/). |
| Authentication | Configure model provider(s) per omp's docs (Anthropic, OpenAI, Gemini, etc.) — set the relevant `*_API_KEY`, or run `omp token <provider>` for OAuth. |
| Notes | Set a daemon-wide default model with `MULTICA_OMP_MODEL` (a `provider/id` selector from `omp models --json`). |
